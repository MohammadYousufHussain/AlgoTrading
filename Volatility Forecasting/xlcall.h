...
struct xloper 
{
   union 
   {
      ...
      LPSTR str;                      /* xltypeStr */
      WORD xlbool;                    /* xltypeBool */
      WORD boolean;                   /* alias for xlbool */
      WORD err;                       /* xltypeErr */
      ...
      struct
      {
         union
         {
            // BYTE far *lpbDatax;      /* data passed to XL */  removed by andyst on 4/5/95
            BYTE far *lpbData;      /* data passed to XL     added by andyst on 4/5/95   */
...
typedef struct xloper XLOPER, *LPXLOPER;
...
// These bits could be set on top of any of the types above.
...
//
// The following copyright notice and disclaimer applies to class XLOper below.
//
// ======================================================================================
...
public:
   ...
   ~XLOper();
   template<typename T> static XLOper* New(T t){ XLOper*p = new XLOper(t); p->xltype |= xlbitDLLFree; return p; }
   static void xLLFree(XLOPER &p);
   static void xLLFree(LPXLOPER p);
   static XLOper XLError(int error_type);
   static XLOper xlErrValue;
};
